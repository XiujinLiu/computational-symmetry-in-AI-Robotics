...
import math
from egnn import E_GCL, get_edges_batch
...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Ensure x is of shape (seq_len, batch_size, d_model) for MultiheadAttention
        x = x.permute(1, 0, 2)  # Convert to (seq_len, batch_size, d_model)

        attn_output, _ = self.attn(x, x, x)  # (seq_len, batch_size, d_model)
...
class GraphNetAutoCenter(nn.Module):
    def __init__(self, auto_offset=True, auto_offset_MLP_depth_list=[256, 64, 3], edge_MLP_depth_list=[259, 256, 256], update_MLP_depth_list=[256, 256, 256]):
        ...
        # self.offset_posi = PositionalEncoding(3)
        self.offset_att = SelfAttentionLayer(3, 3)
        self.output_att = SelfAttentionLayer(update_MLP_depth_list[-1], 4)
    ...
    def forward(self, input_vertex_features,
        ...
        input_vertex_coordinates_reshaped = input_vertex_coordinates.unsqueeze(0)
        # input_vertex_coordinates = self.offset_att(input_vertex_coordinates_reshaped).squeeze(0)
        ...
        # output_vertex_features = self.output_att(output_vertex_features.unsqueeze(0)).squeeze(0)
        return output_vertex_features, edge_features

class ClassAwarePredictor(nn.Module):
    def __init__(self, num_classes, box_encoding_len):
        super(ClassAwarePredictor, self).__init__()
        self.cls_fn = multi_layer_fc_fn(Ks=[256, 64], num_layers=2, num_classes=num_classes, is_logits=True)
        ...
        for i in range(num_classes):
            self.loc_fns += [
                    multi_layer_fc_fn(Ks=[256, 256, 64], num_layers=3, num_classes=box_encoding_len, is_logits=True)]
...
class MultiLayerFastLocalGraphModelV2(nn.Module):
    def __init__(self, num_classes, box_encoding_len, regularizer_type=None, \
        ...
        self.egnn = E_GCL(input_nf=256, hidden_nf=256, output_nf=256, edges_in_d=2)
        for i in range(graph_net_layers):
            self.graph_nets.append(GraphNetAutoCenter())
        for i in range(graph_net_layers-2):
            self.egnns.append(E_GCL(input_nf=256, hidden_nf=256, output_nf=256, edges_in_d=2))
    ...
    def forward(self, batch, is_training):
        ...
        # point_features, edge_features = graph_nets[0](point_features, point_coordinates, keypoint_indices, set_indices)
        for i, graph_net in enumerate(self.graph_nets):
            point_features, edge_features = graph_net(point_features, point_coordinates, keypoint_indices, set_indices)
            if i == 1:
                point_features, _ ,edge_features   = self.egnns[i-1](h=point_features, edge_index=set_indices, coord=point_coordinates, edge_attr=edge_features)
