...
if __name__ == "__main__":
    ...
    for epoch in range(1, epoches):
        ...
        #for batch_idx in range(0, NUM_TEST_SAMPLE-batch_size+1, batch_size):
            ...
            # t_total_loss = w1*t_cls_loss + w2*t_loc_loss + w3*t_reg_loss
            # if start_finetune == False:
            #     mu = max((2*((max(t_cls_loss.item(), t_loc_loss.item(), t_reg_loss.item())*100)**2))/c_**2, 1.0)
            #     start_finetune = True
            # else:
            #     mu = max(mu/1.4, 1.0)
            ...
            # t_total_loss.backward(retain_graph=True)
            optimizer.step()
            # w1 = ((mu * c_**2) / ((t_cls_loss.item()*100 + mu * c_**2))**2)
            # w2 = ((mu * c_**2) / ((t_loc_loss.item()*100 + mu * c_**2))**2)
            # w3 = ((mu * c_**2) / ((t_reg_loss.item()*100 + mu * c_**2))**2)
            ...
            for i in range(NUM_CLASSES):
                ...
                mAP_list[i] += [mAPs[i]]
            avg_mAP = np.mean([np.mean(mAP_list[i]) for i in range(NUM_CLASSES)])
        ...
        if avg_mAP > best_mAP:
            ...
            print(f"Best model saved from epoch {epoch} with mAP: {best_mAP:.4f}")
        # epoch model
